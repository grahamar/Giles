...
object DefaultAmazonS3Client {
  ...
  private def upload(file: File, filename: Option[String] = None): Boolean = {
    try {
      ...
      Logger.info(s"Uploading index '${file.getAbsolutePath}' -> '$s3Filename' to S3.")
  ...
  private def download(file: File, filename: String): Boolean = {
    try {
      Logger.info(s"Downloading index '${file.getAbsolutePath}' from S3.")
  ...
  private def delete(fileKeyName: String): Boolean = {
    try {
      Logger.info(s"Deleting S3 file '$fileKeyName'.")
  ...
  private def uploadOrOverwrite(file: File, filename: Option[String] = None): Boolean = {
    if(doesFileExist(filename.getOrElse(file.getName))) {
      Logger.info("Remote index exists, deleting...")
      if(delete(filename.getOrElse(file.getName))) {
        Logger.info("Deleted remote index, uploading new index...")
        val uploaded = upload(file, filename)
        if(uploaded) {
          Logger.info("Remote index backed up.")
        } else {
          Logger.error("Remote index not uploaded!")
        }
        uploaded
      } else {
        Logger.error("Couldn't remove existing remote index!")
        false
      }
    } else {
      Logger.info("Remote index doesn't exist, uploading...")
      val uploaded = upload(file, filename)
      if(uploaded) {
        Logger.info("Remote index backed up.")
      } else {
  ...
  def tryRestoreIndex(file: File) = {
    ...
    if(tryRestoreOnStart && !file.exists() && remoteIndexExists) {
      Logger.info("Restoring index from S3.")
      withTemporaryFile("giles_index_backup", ".zip") { tmpFile =>
